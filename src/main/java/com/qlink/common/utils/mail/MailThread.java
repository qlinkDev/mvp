...
public class MailThread extends Thread {
	...
	private String type;				// 邮件类型，如果不为空一则表示需要记录失败记录，二则表示记录类型
	
	@Autowired
	private static MailFailureRecordService failureRecordService = SpringContextHolder.getBean(MailFailureRecordService.class);
	...
	private void sendMail() {
    	try {
			...
			// account
			//String account = DictUtils.getDictValue(Constants.DICT_MAIL_ACCOUNT_LABEL, Constants.DICT_MAIL_ACCOUNT_TYPE, "shuxinjm@126.com");
			String account = propertiesLoader.getProperty("account");
			// password
			//String password = DictUtils.getDictValue(Constants.DICT_MAIL_PASSWORD_LABEL, Constants.DICT_MAIL_PASSWORD_TYPE, "shuxin19871112qq");
			...
			saveMailCord(MailFailureRecord.STATUS_SUCCESS);
			
		} catch (Exception e) {
			logger.info("收件人:{},邮件内容：{},发送结果：{}", sendTos, content, "邮件发送失败!");
			e.printStackTrace();
			saveMailCord(MailFailureRecord.STATUS_FAIL);
		}
	}
	...
	private void saveMailCord(String status){
		MailFailureRecord record = new MailFailureRecord();
		record.setEmails(this.sendTos);
		record.setStatus(status);
		record.setTitle(subject);
		record.setNickname(nickName);
		record.setBody(content);
		record.setType(MailFailureRecord.TYPE_JSONRPC);
		failureRecordService.save(record);
	}
