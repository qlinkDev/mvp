...
/*********************** Qlc消耗相关智能合约 ***********************/
/// @title Qlc数据资产
contract QlcDataDictionaryContract {
	
	mapping (address => uint256) public szzcBalanceOf;	// 地址和地址所具有的QLC数量数组
	...
	function reduce(address _target, uint256 _value) internal returns (bool success) {
		require (_value < szzcBalanceOf[_target]);     // 检查余额是否足够
		szzcBalanceOf[_target] -= _value;
		return true;
	}
}
...
contract QlcRecordContract {
    ...
    // 存储设备流量数据
    function qlcRecord_add(address record_address, uint record_type, uint amount, uint time) {   		
   		// 存储
...
/// @title 设备流量记录
contract DeviceFlowRecordContract is QlcRecordContract, QlcDataDictionaryContract {
    ...
    uint flow_sign = 100;	// 由于栈深的限制把流量间隔作常量处理
	
    // 存储设备流量数据
    function deviceFlowRecord_add(bytes32 imei, address user_address, uint time, uint used_flow, uint flow_ratio, uint time_ratio, uint time_sign) {
        	...
        	if (deviceFlowRecords[i].imei == imei) {
        		...
        		uint flow_qlc = 0;	// 流量消耗QLC
	   	 		...
	   	 		if (qlc > 0) {
	   	 			...
	   	 			// 记录数据资产交易记录
	   	 			qlcRecord_add(user_address, 1, qlc, time);
	   	 		}
	   	 		break;
       		}
   	 	}
...
contract MyAdvancedToken is owned, token, DeviceFlowRecordContract {
	...
	function rechargeDataDictionary(address _from, address _to, uint256 _value) returns (bool success) {
		...
		// 记录数据资产交易记录
   	 	qlcRecord_add(_from, 0, _value, now);
		return true;
	}
